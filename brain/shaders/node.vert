...
void main()
{
    ...
    vec3 drift = vec3(
        sin(u_time * 0.5 + id * 0.7) * 0.8,
        cos(u_time * 0.3 + id * 1.1) * 0.6,
        sin(u_time * 0.4 + id * 0.9) * 0.5
    );

    // Subtle size pulsation per node
    float pulse = 1.0 + 0.12 * sin(u_time * 1.5 + id * 2.3);
    ...
    vec3 world_pos = a_position + drift + cam_right * a_quad.x * size + cam_up * a_quad.y * size;
